...
def parse_args():
    ### Option defining
    parser = argparse.ArgumentParser(prog="pcoc_det_mod.py", description='')
...
# this method takes:
# (a) a tree (ete3 object), MSA (BioPython object) and scenario dict()
# (b) an Argparser.Namespace() of command line args, complete with defaults
# (c) an unparsed sys.argv string from the command line
# The inputs are checked in that order: objects take precedence over Namespace over string.
def main(args):
    ...
    ali = load_ali(ali_filename, logger)
    ...
    t, tree = load_tree(tree_filename, logger)

    # check agreement of taxa
    tree_ali_agree(t, ali, logger)

    # if scenario is encoded in Newick file
    if args.manual_mode == "-":
        manual_mode_nodes = scenario_tree_to_dict(t, tree_filename, logger)
    # if scenario is encoded in a string
    elif args.manual_mode:
        p_events, manual_mode_nodes = scenario_string_to_dict(args.manual_mode)
    ...
    # make temp directories if needed and return their names
    #repbppconfig, repseq, ali_basename, repest0, reptree0, repfasta0 = get_temp_dirs(ali_filename, OutDirName)

    # The laundry list of keyword parameters below all used to be global vars.
    # They will be removed as I decompose main() and mk_detect().
    mk_detect(manual_mode_nodes, tree_filename, ali,
              *get_temp_dirs(ali_filename, OutDirName),
              #repest0 = repest0,
              #reptree0 = reptree0,
              #repfasta0 = repfasta0,
...
# NOTE: I think tree_filename can be a Newick string rather than a filename!
def mk_detect(manual_mode_nodes, tree_filename, ali,
              # ordered to take input from *get_temp_dirs(ali_filename, OutDirName):
              repbppconfig, repseq, ali_basename, repest0, reptree0, repfasta0,
              NbCat_Est = 10,
              date = datetime.datetime.now().strftime("%Y%m%d_%H%M%S"),
              dict_p_filter_threshold = {},
              positions_to_highlight = [],
    ...
    # because this method ultimately traces back to init_tree() and the Tree() method,
    # it will work just as well with a Newick string as tree_filename
    ...
    (allbranchlength, convbranchlength) = g_tree.mk_tree_for_simu(plot = plot)
...
#$      bpp_lib.make_estim(ali_basename, nodesWithAncestralModel, nodesWithTransitions, nodesWithConvergentModel, e1, e2, repseq, tree_fn, repest, repbppconfig, NBCATest=NbCat_Est, suffix="_noOneChange",  OneChange = False, ext="", max_gap_allowed=args.max_gap_allowed, gamma=args.gamma, inv_gamma=args.inv_gamma)
        bpp_lib.make_estim(ali_basename, e1, e2, g_tree, NBCATest=NbCat_Est, suffix="_noOneChange",  OneChange = False, ext="", max_gap_allowed=max_gap_allowed, gamma=gamma, inv_gamma=inv_gamma)
#$      bpp_lib.make_estim(ali_basename, nodesWithAncestralModel, nodesWithTransitions, nodesWithConvergentModel, e1, e2, repseq, tree_fn, repest, repbppconfig, NBCATest=NbCat_Est, suffix="_withOneChange",  OneChange = True, ext="", max_gap_allowed=args.max_gap_allowed, gamma=args.gamma, inv_gamma=args.inv_gamma)
        bpp_lib.make_estim(ali_basename, e1, e2, g_tree, NBCATest=NbCat_Est, suffix="_withOneChange",  OneChange = True, ext="", max_gap_allowed=max_gap_allowed, gamma=gamma, inv_gamma=inv_gamma)
    ...
    # if an ID is supplied to save the results
    if prefix_out:
    ...
    # if args are passed and they say not to delete the temp files
    if args and not args.no_cleanup:
        remove_folder(g_tree.repest)
        remove_folder(g_tree.repbppconfig)
        remove_folder(g_tree.reptree)
        if args and not args.no_cleanup_fasta:
            remove_folder(g_tree.repfasta)

    metadata_simu_dico["time"] = str(time.time() - start_detec)

    # return the final unfiltered dataframe
    return g_tree, df_bilan
...
def scenario_string_to_dict(scenario_string):
    manual_mode_nodes = {"T": [], "C": []}
    p_events = scenario_string.strip().split("/")
    for e in p_events:
        l_e = map(int, e.split(","))
...
def load_tree(tree_filename, logger):
    logger.info("tree: %s", tree_filename)
...
def scenario_tree_to_dict(t, tree_filename, logger):
    manual_mode_nodes = {"T": [], "C": []}
    features = []
    ...
    if not "Condition" in features or not "Transition" in features:
        logger.error(
            "\"Transition\" and \"Condition\" tags are not detected. Use a valid tree are use a manual scenario in the \"-m\" option.")
        sys.exit(1)
    ...
    return manual_mode_nodes

def load_ali(ali_filename, logger):

    logger.info("alignment: %s", ali_filename)
    ...
    if os.path.isfile(ali_filename):
        ...
        for seq in ali:
            for letter in seq.seq:
                n_tot += 1
                if letter in ["-", "X", "A", "C", "D", "E", "F", "G", "H", "I", "K", "L", "M", "N", "P", "Q", "R", "S",
                              "T", "V", "W", "Y"]:
        ...
        logger.info("Alphabet usage:\n%s ",
                    ", ".join(["%s(%.2f%%)" % (k, float(100 * v / float(n_tot))) for k, v in alphabet.items()]))
        if error:
            logger.error("Invalid character in the alignment: %s",
                         ", ".join(["%s(%.2f%%)" % (k, float(100 * v / float(n_tot))) for k, v in error.items()]))
            sys.exit(1)
    else:
...
def tree_ali_agree(t, ali, logger):
...
if __name__ == "__main__":
    ...
    args = parse_args()
    # run body of standalone program
    main(args)
